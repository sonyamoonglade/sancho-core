...
import { AppRoles, Categories, DatabaseCartProduct, OrderStatus, Product, ResponseUserOrder } from "../../src/common/types";
...
import { QueryBuilder } from "../../src/server/packages/query_builder/QueryBuilder";
...
describe("OrderController (e2e)", () => {
   ...
   const mockSessions = [mockSession, mockSessionWithWaitingOrder, mockWorkerSession];
   ...
   it("/createUserOrder (POST) puts session of user which has got waiting order. Should return 400", async () => {
      ...
      return request(app.getHttpServer()).post("/api/v1/order/createUserOrder").set("Cookie", mockSIDCookie).send(dto).expect(400, {
   ...
   it("/cancel (PUT) user cancels order (with cancel_ban) should return 403", () => {
      ...
      return request(app.getHttpServer()).put("/api/v1/order/cancel").set("Cookie", mockSIDCookie).send(dto).expect(403);
   });
